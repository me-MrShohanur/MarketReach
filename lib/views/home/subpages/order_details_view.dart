...
class _Header extends StatelessWidget {
  ...
  void _saveOrder(BuildContext context, OrderDetailLoaded state) async {
    ...
    try {
      // Build updated details with new quantities
      final List<Map<String, dynamic>> updatedDetails = order.details.map((d) {
        final newQty = editedFields['qty_${d.productId}'];
        final qty = newQty != null
            ? double.tryParse(newQty) ?? d.unitQty
            : d.unitQty;
      ...
      final response = await OrderSaveService.saveOrder(
        ...
        orderId: order.orderId, // ← PASS THE ORDER ID HERE
      );
          ...
          children: [
            ListView(
              ...
              children: [
                ...
                // ── Products Card (WITH QUANTITY EDITING) ───────────────────────────
                _SectionCard(
                  accent: Colors.black,
...
// ═══════════════════════════════════════════════════════════════════════════════
// EDITABLE PRODUCT ITEM (Only Quantity)
// ═══════════════════════════════════════════════════════════════════════════════

class _EditableProductItem extends StatelessWidget {
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  _Chip(
                    label: 'Pcs',
                    value: '${item.pcsQty ?? item.unitQty}',
                    color: Colors.teal,
                  ),
                  _Chip(
                    label: 'Rate',
