class ProductModel {
  ...
  final double
  cartDiscount; // discount PERCENTAGE (e.g. 15.0 means 15%) → maps to "discount" in API
  final double
  cartDiscountAmt; // actual BDT discount amount (qty × rate × disc%) → maps to "discountAmt" in API
  final double
  cartNetAmount; // calculated: (cartQty × cartRate) − cartDiscountAmt
  ...
  // copyWithCart — creates a new ProductModel with updated cart fields.
  // Called in _AddProductDetailSheet when user taps Save.
  //
  // cartDiscount    = the % value  (e.g. 15.0)   → "discount"    in API body
  // cartDiscountAmt = the BDT amount (e.g. 4980.0) → "discountAmt" in API body
  ProductModel copyWithCart({
    required double cartQty,
    required double cartRate,
    required double cartDiscount,
    required double cartDiscountAmt,
